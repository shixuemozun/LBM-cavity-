...
double rho[NX+1][NY+1],u[NX+1][NY+1][2],F[NX+1][NY+1][Q],f[NX+1][NY+1][Q],feq[NX+1][NY+1][Q]; 
...
cout<<"循环次数：" << t <<"  " <<"进度：" << (double)t/150000<<endl; 
...
F[i][j][k]=f[ip][jp][k]+(feq[ip][jp][k]-f[ip][jp][k])/0.8;	  //不正确的松弛时间会导致计算发散 
}
}	
}
 	...
 	for(k=0;k<Q;k++)
		 {
			 ...
			 rho[0][j]=rho[1][j];
 	        f[0][j][k] = feq[0][j][k]-feq[1][j][k]+f[1][j][k];
...
for(i=0;i<=NX;i++)//上下边界
{
	...
	rho[i][0]=rho[i][1];
	f[i][0][k] = feq[i][0][k]-feq[i][1][k]+f[i][1][k];
	

	rho[i][NY]=rho[i][NY-1];
	f[i][NY][k] = feq[i][NY][k] - feq[i][NY-1][k]+f[i][NY-1][k];
